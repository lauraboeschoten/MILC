...
Qbar3 <- mean(Q3hat1, Q3hat2, Q3hat3, Q3hat4, Q3hat5)
Qbar4 <- mean(Q4hat1, Q4hat2, Q4hat3, Q4hat4, Q4hat5)

Ubar1 <- mean(U1hat1, U1hat2, U1hat3, U1hat4, U1hat5)
Ubar2 <- mean(U2hat1, U2hat2, U2hat3, U2hat4, U2hat5)
Ubar3 <- mean(U3hat1, U3hat2, U3hat3, U3hat4, U3hat5)
Ubar4 <- mean(U4hat1, U4hat2, U4hat3, U4hat4, U4hat5)
...
ll3 <- Qbar3 - qt(.975, sum(table(data$toeslag))-1)*sqrtT1
ll4 <- Qbar4 - qt(.975, sum(table(data$toeslag))-1)*sqrtT1

ul1 <- Qbar1 + qt(.975, sum(table(data$toeslag))-1)*sqrtT1
ul2 <- Qbar2 + qt(.975, sum(table(data$toeslag))-1)*sqrtT1
ul3 <- Qbar3 + qt(.975, sum(table(data$toeslag))-1)*sqrtT1
ul4 <- Qbar4 + qt(.975, sum(table(data$toeslag))-1)*sqrtT1
...
round(model3, 4)
#       Qbar  Ubar B     T  sqrtT      ll     ul     ci
#[1,] 0.6791 3e-04 0 3e-04 0.0167  0.6462 0.7119 0.0657
#[2,] 0.0205 0e+00 0 0e+00 0.0051 -0.0123 0.0534 0.0657
#[3,] 0.2953 0e+00 0 0e+00 0.0043  0.2624 0.3281 0.0657
#[4,] 0.0000 0e+00 0 0e+00 0.0051 -0.0328 0.0328 0.0657
